...
func TestStoreImpl_PatchFHIREncounter(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				...
				encounterID: gofakeit.UUID(),
				input: domain.FHIREncounterInput{
					Status: domain.EncounterStatusEnumCancelled,
				},
			},
			wantErr: true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := fh.PatchFHIREncounter(tt.args.ctx, tt.args.encounterID, tt.args.input)
			if (err != nil) != tt.wantErr {
...
func TestStoreImpl_GetFHIRComposition(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.name == "happy case: get composition" {
				dataset.MockGetFHIRResourceFn = func(resourceType, fhirResourceID string, resource interface{}) error {
					id := gofakeit.UUID()
					...
					bs, err := json.Marshal(composition)
					if err != nil {
						return err
					}
