...
func TestGraphQFindPatientsByMSISDN(t *testing.T) {
	...
	_, _ = usecase.DeleteFHIRPatientByPhone(ctx, payload.PhoneNumber)
}
...
func TestGraphQLStartEpisodeByOTP(t *testing.T) {
	...
	if err != nil {
		log.Errorf("error in generating test OTP: %v", err)
		return
	}
...
func TestGraphQLUpgradeEpisode(t *testing.T) {
	...
	otp, err := generateTestOTP(t, msisdn)
	if err != nil {
		log.Errorf("unable to get verified phone number and OTP %v", err)
		return
	}
...
func TestGraphQLPatientMedicalData(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				query: map[string]interface{}{
					"query": `query medicalData($patientID: String!) {
